...
impl __LensoIntoOpenaiCodexAccessResult for Result<Result<AccessResponse, AccessError>, RuntimeFailure> {
    fn __lenso_into_result(self) -> Result<Result<AccessResponse, AccessError>, RuntimeFailure> { self }
}
impl __LensoIntoOpenaiCodexAccessResult for Result<AccessResponse, lenso_plugin_authoring::PluginError<AccessError, RuntimeFailure>> {
    fn __lenso_into_result(self) -> Result<Result<AccessResponse, AccessError>, RuntimeFailure> {
        match self {
...
macro_rules! __lenso_native_lower_openai_codex {
    ($plugin:ty, $support:path) => {
        ...
        impl $crate::OpenaiCodexProvider for $plugin {
...
#[doc(hidden)]
#[macro_export]
macro_rules! __lenso_native_endpoints_openai_codex {
...
macro_rules! __lenso_native_provide_openai_codex {
    ($provider:expr, $lifecycle:expr, $support:path) => {{
        ...
        let (request_endpoints, stream_endpoints, event_endpoints) =
            $crate::__lenso_native_endpoints_openai_codex!($provider, $support);
...
impl CapabilityClientMany for OpenaiCodexClient {
        ...
        dependencies
            ...
            .map(|binding| {
                Ok(BoundCapabilityClient::new(
                    ...
                    Self {
                    access: binding.handle().ok_or(RuntimeFailure::Unavailable { capability: CAPABILITY_ID })?.typed::<OpenaiCodex>()?,
                    },
                ))
            })
            .collect()
