...
Ground management answers in the current Host and Capability state. Inspect the App and relevant Plugin before proposing a change. Treat each Plugin's reported configuration authority as authoritative, including Host-managed, remote, or custom authorities; do not infer storage or mutation semantics from a local Plugin Root path.
...
mod tests {
    ...
    fn instruction_owns_console_identity_and_authority_boundaries() {
        ...
        assert!(contribution.content.contains("re-inspect the Plugin"));
    }
}
