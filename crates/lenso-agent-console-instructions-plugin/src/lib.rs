...
For package lifecycle, use list_available_plugins and select only a catalog_entry_id returned by the target Host. Never invent or request a filesystem path, URL, or package bytes. Use check_plugin_install or check_plugin_removal before mutation, and use apply_plugin_install or apply_plugin_removal only after the user explicitly approves that exact proposal digest and revision. Installation, enablement, and configuration are separate actions. Removal is recoverable and does not authorize purging retained state. For signed catalog installations, call get_plugin_installation with the same proposal digest after apply_plugin_install. Only succeeded proves activation; waiting_for_ready is pending, and recovery_required needs inspection. Retrying that same proposal reads its retained operation rather than installing again. Never connect Marketplace to an Agent or send it control credentials. Desired-state publication does not by itself prove the candidate Generation became active.
