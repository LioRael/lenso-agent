...
pub struct TaskSnapshot {
    ...
    #[serde(deserialize_with = "lenso_contract_runtime::serde::deserialize_required")]
    pub task_id: String,
    #[serde(rename = "terminal_result")]
    #[serde(default)]
    #[serde(skip_serializing_if = "Option::is_none")]
    #[serde(deserialize_with = "lenso_contract_runtime::serde::deserialize_optional_value")]
    pub terminal_result: OptionalValue<TerminalResult>,
...
impl RequestCapability for TaskSupervisor {
    ...
    fn invoke_native(endpoint: &dyn NativeRequestEndpoint, operation: &str, request: Self::Request, context: InvocationContext) -> NativeRequestFuture<Self> {
        ...
        let Some(typed_endpoint) = endpoint
            .typed_endpoint()
            .and_then(|endpoint| endpoint.downcast_ref::<TaskSupervisorRequestEndpoint>())
        else {
            return lenso_kernel::invoke_typed_or_erased_native_request::<Self>(endpoint, operation, request, context);
        };
...
#[doc(hidden)]
pub trait __LensoIntoTaskSupervisorSnapshotResult {
    fn __lenso_into_result(self) -> Result<Result<SnapshotResponse, SnapshotError>, RuntimeFailure>;
}
impl __LensoIntoTaskSupervisorSnapshotResult for Result<SnapshotResponse, SnapshotError> {
    fn __lenso_into_result(self) -> Result<Result<SnapshotResponse, SnapshotError>, RuntimeFailure> { Ok(self) }
}
impl __LensoIntoTaskSupervisorSnapshotResult for Result<Result<SnapshotResponse, SnapshotError>, RuntimeFailure> {
    fn __lenso_into_result(self) -> Result<Result<SnapshotResponse, SnapshotError>, RuntimeFailure> { self }
}
impl __LensoIntoTaskSupervisorSnapshotResult for Result<SnapshotResponse, lenso_plugin_authoring::PluginError<SnapshotError, RuntimeFailure>> {
    fn __lenso_into_result(self) -> Result<Result<SnapshotResponse, SnapshotError>, RuntimeFailure> {
        match self {
...
impl __LensoIntoTaskSupervisorSnapshotResult for Result<SnapshotResponse, TaskSupervisorInvocationError> {
    fn __lenso_into_result(self) -> Result<Result<SnapshotResponse, SnapshotError>, RuntimeFailure> {
        match self {
            Ok(value) => Ok(Ok(value)),
            Err(TaskSupervisorInvocationError::Domain(error)) => Ok(Err(error)),
            Err(TaskSupervisorInvocationError::Runtime(error)) => Err(error),
        }
    }
}

pub trait TaskSupervisorProvider: fmt::Debug + 'static {
    fn snapshot(&self, context: InvocationContext, request: SnapshotRequest) -> NativeRequestFuture<TaskSupervisor>;
}

#[doc(hidden)]
#[macro_export]
macro_rules! __lenso_native_lower_task_supervisor {
    ($plugin:ty, $support:path) => {
        use $support as __LensoNativeSupportTaskSupervisor;
        impl $crate::TaskSupervisorProvider for $plugin {
...
#[derive(Debug)]
pub struct TaskSupervisorEndpoint<P: TaskSupervisorProvider> { provider: Rc<P>, request_endpoint: TaskSupervisorRequestEndpoint }
impl<P: TaskSupervisorProvider> TaskSupervisorEndpoint<P> {
    pub fn new(provider: P) -> Self {
        let provider = Rc::new(provider);
        let request_provider: Rc<dyn TaskSupervisorProvider> = provider.clone();
        Self { provider, request_endpoint: TaskSupervisorRequestEndpoint { provider: request_provider } }
    }
}

impl<P: TaskSupervisorProvider> NativeRequestEndpoint for TaskSupervisorEndpoint<P> {
...
#[doc(hidden)]
#[macro_export]
macro_rules! __lenso_native_endpoints_task_supervisor {
    ($provider:expr, $support:path) => {{
        use $support as __LensoNativeSupport;
        let endpoint = ::std::rc::Rc::new($crate::TaskSupervisorEndpoint::new($provider));
        (
...
#[doc(hidden)]
#[macro_export]
macro_rules! __lenso_native_provide_task_supervisor {
    ($provider:expr, $lifecycle:expr, $support:path) => {{
        use $support as __LensoNativeSupport;
        let (request_endpoints, stream_endpoints, event_endpoints) =
            $crate::__lenso_native_endpoints_task_supervisor!($provider, $support);
...
impl TaskSupervisorClient {
    pub fn new(handle: NativeRequestHandle<TaskSupervisor>) -> Self {
    ...
    pub async fn snapshot(&self, request: SnapshotRequest) -> Result<SnapshotResponse, TaskSupervisorInvocationError> {
        self.snapshot.invoke(SNAPSHOT_OPERATION, request).await
            ...
            .map_err(TaskSupervisorInvocationError::Domain)
    }

    pub async fn snapshot_with_context(&self, context: InvocationContext, request: SnapshotRequest) -> Result<SnapshotResponse, TaskSupervisorInvocationError> {
...
impl CapabilityClient for TaskSupervisorClient {
    ...
    fn from_dependencies(dependencies: &PluginDependencies) -> Result<Self, RuntimeFailure> {
        Ok(Self {
            snapshot: dependencies.one::<TaskSupervisor>()?,
        })
    }
...
impl CapabilityClientMany for TaskSupervisorClient {
        ...
        dependencies
            ...
            .map(|binding| {
                Ok(BoundCapabilityClient::new(
                    ...
                    Self {
                    snapshot: binding.handle().ok_or(RuntimeFailure::Unavailable { capability: CAPABILITY_ID })?.typed::<TaskSupervisor>()?,
                    },
                ))
            })
            .collect()
...
#[derive(Clone, Debug, PartialEq)]
pub enum TaskSupervisorInvocationError {
    Domain(SnapshotError),
    Runtime(RuntimeFailure),
}

#[derive(Clone, Copy, Debug)]
pub struct TaskSupervisorGuestClient<'a, H: lenso_guest_sdk::HostImports> {
    capability: lenso_guest_sdk::GuestCapability<'a, H>,
}

impl<'a, H: lenso_guest_sdk::HostImports> TaskSupervisorGuestClient<'a, H> {
...
pub struct TaskSupervisorJsonCodec;

impl lenso_runtime_codec::JsonCapabilityCodec for TaskSupervisorJsonCodec {
    fn capability_id(&self) -> &'static str { CAPABILITY_ID }
    ...
    fn invoke_host_request(&self, dependency: lenso_kernel::PluginDependencyHandle, operation: String, request: serde_json::Value, context: InvocationContext) -> lenso_runtime_codec::JsonHostRequestFuture {
        match operation.as_str() {
            SNAPSHOT_OPERATION => {
                ...
                Box::pin(async move {
                    let request = request?;
                    let handle = dependency.typed::<TaskSupervisor>()?;
