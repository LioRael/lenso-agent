//! Removable, stateless text Tool Provider Plugin Plugin.
...
mod tests {
    ...
    fn generated_provider_derives_schema_and_dispatches_safely() {
        ...
        assert!(matches!(
            unknown,
            Err(PluginError::Domain(ExecuteError::NotFound))
        ));
    ...
    #[test]
    fn generated_plugin_descriptor_is_package_owned_and_complete() {
        let descriptor: serde_json::Value =
            serde_json::from_str(PLUGIN_DESCRIPTOR_JSON).expect("descriptor must be valid JSON");
        assert_eq!(descriptor["plugin_id"], "lenso.agent.text-tools");
        assert_eq!(descriptor["release_version"], env!("CARGO_PKG_VERSION"));
        assert_eq!(
