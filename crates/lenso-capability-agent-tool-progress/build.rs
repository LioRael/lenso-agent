...
fn main() {
    ...
    if env::var_os("LENSO_UPDATE_CONTRACT_SNAPSHOT").is_some() {
        write_source_snapshot(&snapshot, Path::new("capability.json"))
            .unwrap_or_else(|error| panic!("failed to update Tool Progress snapshot: {error}"));
    } else {
        check_source_snapshot(&snapshot, Path::new("capability.json"))
            .unwrap_or_else(|error| panic!("Tool Progress contract artifacts are stale: {error}"));
    }
    ...
    .unwrap_or_else(|error| panic!("Tool Progress generated artifacts are stale: {error}"));
}
