//! Bounded filesystem-backed Skill catalog and progressive-disclosure Provider Plugin.
...
#[derive(Clone, Debug)]
struct FilesystemSkillsPlugin {
    #[config]
    config: SkillsConfig,
    provider: FilesystemSkillsProvider,
}

impl Lifecycle for FilesystemSkillsPlugin {
...
impl FilesystemSkillsPlugin {
    ...
    ) -> impl std::future::Future<Output = PluginResult<ExecuteResponse, ExecuteError>> {
        let result = self.provider.execute_now(&request);
        drop(request);
        std::future::ready(match result {
            Ok(response) => Ok(response),
            Err(ProviderFailure::Domain(error)) => Err(PluginError::domain(error)),
            Err(ProviderFailure::Runtime(error)) => Err(PluginError::runtime(error)),
        })
    }
    ...
    ) -> impl std::future::Future<Output = PluginResult<ContributeResponse, ContributeError>> {
        let result = self
            .provider
            .state
            .borrow()
...
fn canonical_root(configured: &Path) -> Result<PathBuf, RuntimeFailure> {
    ...
    if !root.is_dir() {
        return Err(plugin_failure("filesystem Skills root is not a directory"));
    }
    Ok(root)
}
...
fn discover_skills(
    ...
    let entries = fs::read_dir(root).map_err(|error| {
        plugin_failure(format!("filesystem Skills root is unreadable: {error}"))
    })?;
    ...
    for entry in entries {
        ...
        let name = entry
            .file_name()
            .into_string()
            .map_err(|_| plugin_failure("filesystem Skill directory name is not valid UTF-8"))?;
        if name.starts_with('.') {
            continue;
        }
...
fn load_skill(
    ...
    let skill_root = resolved.parent().ok_or_else(|| {
        plugin_failure(format!("filesystem Skill `{directory_name}` has no root"))
    })?;
...
fn expand_home(path: &Path) -> Result<PathBuf, RuntimeFailure> {
    ...
    if value == "~" {
        return BaseDirs::new()
            ...
            .ok_or_else(|| plugin_failure("home directory is unavailable"));
    }
...
fn relative_resource_path(path: &Path) -> Result<String, RuntimeFailure> {
        ...
        path.components()
            .map(|component| match component {
                Component::Normal(part) => part.to_str().map(ToOwned::to_owned).ok_or_else(|| {
                    plugin_failure("filesystem Skill resource path is not valid UTF-8")
                }),
                _ => Err(plugin_failure(
    ...
    if !valid_resource_path(&value) {
        return Err(plugin_failure("filesystem Skill resource path is invalid"));
    }
    Ok(value)
}
