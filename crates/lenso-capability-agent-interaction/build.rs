...
fn main() {
    ...
    if env::var_os("LENSO_UPDATE_CONTRACT_SNAPSHOT").is_some() {
        write_source_snapshot(&snapshot, Path::new("capability.json"))
            .unwrap_or_else(|error| panic!("failed to update Agent Interaction snapshot: {error}"));
        write_projection(
            Path::new("capability.json"),
            ProjectionLanguage::RustRuntime,
            Path::new("src/generated.rs"),
        )
        .unwrap_or_else(|error| panic!("failed to update Agent Interaction projection: {error}"));
    } else {
        check_source_snapshot(&snapshot, Path::new("capability.json")).unwrap_or_else(|error| {
            panic!("Agent Interaction Descriptor or Schemas are stale: {error}")
        });
    }
    ...
    .unwrap_or_else(|error| panic!("Agent Interaction generated artifacts are stale: {error}"));
}
