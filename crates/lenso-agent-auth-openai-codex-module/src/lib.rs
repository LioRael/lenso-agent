...
impl AuthConfig {
    ...
    fn credential_path(&self) -> Result<PathBuf, RuntimeFailure> {
        ...
        let base = BaseDirs::new()
            .ok_or_else(|| module_failure("the user home directory is unavailable"))?;
    ...
    fn access(
        ...
        Box::pin(async move {
            ...
            let _lock = CredentialLock::acquire(&path)
                .map_err(|_| module_failure("direct-auth credential store is busy"))?;
            ...
            let mut credentials = match read_credentials(&path) {
                ...
                Err(_) => return Err(module_failure("failed to read direct-auth credentials")),
            };
            let Some(mut credential) = credential_from_store(&credentials, &key)
                .map_err(|_| module_failure("direct-auth credential is invalid"))?
            else {
                return Ok(Err(AccessError::NotAuthenticated));
            };
            ...
            if credential.expires_at <= refresh_at {
                credential = match refresh(&client, &config, &credential).await {
                    ...
                    Err(RefreshFailure::Runtime) => {
                        return Err(module_failure("ChatGPT token refresh failed"));
                    }
                };
                credentials.insert(
                    key,
                    serde_json::to_value(&credential)
                        .map_err(|_| module_failure("failed to encode refreshed credentials"))?,
                );
                write_credentials(&path, &credentials)
                    .map_err(|_| module_failure("failed to persist refreshed credentials"))?;
            }
