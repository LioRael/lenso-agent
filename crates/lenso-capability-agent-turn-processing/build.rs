...
fn main() {
    ...
    if env::var_os("LENSO_UPDATE_CONTRACT_SNAPSHOT").is_some() {
        write_source_snapshot(&snapshot, Path::new("capability.json")).unwrap_or_else(|error| {
            panic!("failed to update Agent Turn Processing snapshot: {error}")
        });
        write_projection(
        ...
        .unwrap_or_else(|error| {
            panic!("failed to update Agent Turn Processing projection: {error}")
        });
    } else {
        check_source_snapshot(&snapshot, Path::new("capability.json")).unwrap_or_else(|error| {
            panic!("Agent Turn Processing Descriptor or Schemas are stale: {error}")
        });
    }
    ...
    .unwrap_or_else(|error| panic!("Agent Turn Processing generated artifacts are stale: {error}"));
}
