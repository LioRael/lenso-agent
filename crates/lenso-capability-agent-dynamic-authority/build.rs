...
fn main() {
    ...
    if env::var_os("LENSO_UPDATE_CONTRACT_SNAPSHOT").is_some() {
        write_source_snapshot(&snapshot, Path::new("capability.json")).unwrap_or_else(|error| {
            panic!("failed to update Agent Dynamic Authority snapshot: {error}")
        });
        write_projection(
        ...
        .unwrap_or_else(|error| {
            panic!("failed to update Agent Dynamic Authority projection: {error}")
        });
    } else {
        check_source_snapshot(&snapshot, Path::new("capability.json")).unwrap_or_else(|error| {
            panic!("Agent Dynamic Authority Descriptor or Schemas are stale: {error}")
        });
    }
    ...
    .unwrap_or_else(|error| {
        panic!("Agent Dynamic Authority generated artifacts are stale: {error}")
    });
}
