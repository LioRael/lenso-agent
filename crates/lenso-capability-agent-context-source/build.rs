...
fn main() {
    ...
    if env::var_os("LENSO_UPDATE_CONTRACT_SNAPSHOT").is_some() {
        write_source_snapshot(&snapshot, Path::new("capability.json"))
            .unwrap_or_else(|error| panic!("failed to update Context Source snapshot: {error}"));
        write_projection(
            Path::new("capability.json"),
            ProjectionLanguage::RustRuntime,
            Path::new("src/generated.rs"),
        )
        .unwrap_or_else(|error| panic!("failed to update Context Source projection: {error}"));
    } else {
        check_source_snapshot(&snapshot, Path::new("capability.json"))
            .unwrap_or_else(|error| panic!("Context Source artifacts are stale: {error}"));
    }
    ...
    .unwrap_or_else(|error| panic!("Context Source projection is stale: {error}"));
}
