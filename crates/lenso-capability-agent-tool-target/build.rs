...
fn main() {
    ...
    if env::var_os("LENSO_UPDATE_CONTRACT_SNAPSHOT").is_some() {
        write_source_snapshot(&snapshot, Path::new("capability.json"))
            .unwrap_or_else(|error| panic!("failed to update Tool target snapshot: {error}"));
        write_projection(
            Path::new("capability.json"),
            ProjectionLanguage::RustRuntime,
            Path::new("src/generated.rs"),
        )
        .unwrap_or_else(|error| panic!("failed to update Tool target projection: {error}"));
    } else {
        check_source_snapshot(&snapshot, Path::new("capability.json"))
            .unwrap_or_else(|error| panic!("Tool target Descriptor or Schemas are stale: {error}"));
    }
    check_projection(
        Path::new("capability.json"),
        ProjectionLanguage::RustRuntime,
    ...
    .unwrap_or_else(|error| panic!("Tool target generated artifacts are stale: {error}"));
}
