...
impl<S> __LensoIntoAgentRunTurnStreamResult for Result<S, lenso_plugin_authoring::PluginError<RunTurnError, RuntimeFailure>>
where
    S: NativeStreamSession + 'static,
{
    fn __lenso_into_result(self) -> Result<Box<dyn NativeStreamSession>, AgentInvocationError> {
        match self {
            ...
            Err(lenso_plugin_authoring::PluginError::Domain(error)) => Err(AgentInvocationError::Domain(error)),
            Err(lenso_plugin_authoring::PluginError::Runtime(error)) => Err(AgentInvocationError::Runtime(error)),
        }
    }
}
    ...
    ($plugin:ty, $support:path) => {
        ...
        fn run_turn(&self, context: __LensoNativeSupportAgent::InvocationContext, request: $crate::RunTurnRequest) -> __LensoNativeSupportAgent::LocalBoxFuture<'static, Result<Box<dyn __LensoNativeSupportAgent::NativeStreamSession>, $crate::AgentInvocationError>> {
            ...
            ::std::boxed::Box::pin(async move {
                let result = <$plugin>::run_turn(&plugin, context, request).await;
