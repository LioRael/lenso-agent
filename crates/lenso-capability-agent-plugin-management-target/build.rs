...
fn main() {
    ...
    if env::var_os("LENSO_UPDATE_CONTRACT_SNAPSHOT").is_some() {
        write_source_snapshot(&snapshot, Path::new("capability.json")).unwrap_or_else(|error| {
            panic!("failed to update Plugin management target snapshot: {error}")
        });
        write_projection(
        ...
        .unwrap_or_else(|error| {
            panic!("failed to update Plugin management target projection: {error}")
        });
    } else {
        check_source_snapshot(&snapshot, Path::new("capability.json")).unwrap_or_else(|error| {
            panic!("Plugin management target Descriptor or Schemas are stale: {error}")
        });
    }
    check_projection(
        Path::new("capability.json"),
    ...
    .unwrap_or_else(|error| {
        panic!("Plugin management target generated artifacts are stale: {error}")
    });
}
