...
impl ToolProviderProvider for CodeModeToolsModule {
    fn catalog(
        &self,
    ...
    fn execute(
        ...
        Box::pin(async move { module.run_code(context, arguments.code).await })
    }
}

impl CodeModeToolsModule {
    async fn run_code(
        &self,
        context: InvocationContext,
        code: String,
