"""Run V07 from a copied external consumer source tree."""
...
paths = {
    ...
    "lenso-capability-agent-interaction": agent / "crates/lenso-capability-agent-interaction",
}
for name, path in paths.items():
    assert (path / "Cargo.toml").is_file(), f"missing {name}: {path}"


with tempfile.TemporaryDirectory(prefix="lenso-agent-external-interaction-") as temporary:
    ...
    run(
        ["cargo", "test", "--manifest-path", root / "runner/Cargo.toml", "--test", "realtime"],
        cwd=root,
    )
    print(
        "PASS: a copied external Plugin proved bounded duplex input/output, "
        "independent sequence spaces, half-close, interruption, explicit resume "
        "rejection, and cancellation without a default Agent Loop."
    )
