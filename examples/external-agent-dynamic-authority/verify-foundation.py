"""Run V08 from a copied external consumer source tree."""
...
with tempfile.TemporaryDirectory(prefix="lenso-agent-external-dynamic-authority-") as temporary:
    ...
    run(
        ["cargo", "test", "--manifest-path", root / "runner/Cargo.toml", "--test", "authority"],
        cwd=root,
    )
    print(
        "PASS: a copied external policy Provider selected an exact Tool resource, "
        "sealed a Host-issued grant, revalidated it immediately before execution, "
        "blocked revocation before the side effect, rejected an ordinary forged grant, "
        "and retained static Plan behavior when no grant was selected."
    )
