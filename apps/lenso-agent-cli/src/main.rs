...
async fn run_profile(command: &ProfileCommand) -> Result<(), String> {
    match command {
        ProfileCommand::ImportCoding { url } => import_coding_profiles(url).await,
...
async fn import_coding_profiles(url: &str) -> Result<(), String> {
    let token = std::env::var("LENSO_AGENT_CONTROL_TOKEN").map_err(|_| {
        "LENSO_AGENT_CONTROL_TOKEN is required for online Profile import".to_owned()
    })?;
    ...
    let state = client
        .get(format!("{base}/control/plugins"))
        .bearer_auth(&token)
        .send()
        .await
        .map_err(|error| error.to_string())?
    ...
    let revision = state
        ...
        .ok_or("Plugin management response lacks revision")?;
    ...
    let stream = inventory
        .get("streamId")
        .and_then(serde_json::Value::as_str)
        .ok_or("Plugin management response lacks streamId")?;
    let response = client
        .post(format!("{base}/control/profiles/import"))
        .bearer_auth(token)
        .json(&serde_json::json!({"expectedRevision": revision, "expectedStreamId": stream}))
        .send()
        .await
        .map_err(|error| error.to_string())?;
    ...
    if !status.is_success() {
        return Err(format!("Profile import failed ({status}): {body}"));
    }
    println!("{body}");
    Ok(())
}
