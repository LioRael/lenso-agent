...
use lenso_agent_host::{AgentDirectories, AgentHost, Profile, TelegramSurface};
...
struct Args {
    ...
    /// Durable Telegram update cursor. Defaults below the Agent Home.
    #[arg(long, value_name = "PATH")]
...
async fn run(args: Args) -> Result<(), String> {
    ...
    let state = args
        .state
        .unwrap_or(AgentDirectories::resolve()?.telegram_state());
    let mut options = TelegramOptions::new(token, allowed_chats, state);
