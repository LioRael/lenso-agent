# Tool Hook and approval Plugin cards
...
`lenso plugins configure lenso.agent.approval-hook` creates its Instance file
under `plugins/`; the Host Catalog attaches that Hook provider to both `tools`
and `restricted-read-tools` in the candidate immutable App Generation. The
bundled policy allows `read_text` and asks for every other Tool name. This means
direct Tools, outer `run_code` and `delegate`, Code Mode nested reads, and
child-Agent reads all enter the same Hook mechanism; it does not mean every
call must receive the same decision.

`lenso plugins disable lenso.agent.approval-hook` switches back to a Generation
with no Hook binding.
